...
**현재 primary(구성요소) nomogram의 축 길이 (각 endpoint 최대=100으로 정규화)**

| endpoint | 1위 | 2위 | 3위 | 4위 |
|---|---|---|---|---|
| DSS | Size 100 | Deposit 72 | DOI 56 | PD 53 |
| OS | Size 100 | DOI 76 | Bilateral 62 | Deposit 59 |
| PFS | Deposit 100 | DOI 95 | LN count 68 | PD 61 |
| LRRFS | Deposit 100 | LN count 86 | DOI 81 | PD 49 |

> **주의(중요)**: 축 길이는 `|β| × 값의 "범위"`라서 **이상치/왜도에 민감**합니다.
> 예: `LN count`는 대부분 0~2지만 최대 41이라 **축이 길게** 나옵니다(범위 효과).
> 반면 `PD`·`Deposit`는 **표준화(|β|×SD) 기준으로는 더 중요**합니다(아래 §6 및
...
## 5. 4개 endpoint nomogram 성능 요약 (구성요소 모델, n=133)

| endpoint | 모델 | C-index (corrected) | Calibration slope (3·5년) | Time-AUC (3·5년) | Risk-group log-rank |
|---|---|---|---|---|---|
| **DSS** | 구성요소 8개 | **0.878 (0.860)** [0.815–0.937] | 1.20 / 1.28 | 0.868 / 0.871 | p<0.001 |
| **OS** | 구성요소 8개 | 0.809 (0.792) [0.727–0.875] | 0.89 / 0.94 | 0.787 / 0.782 | p<0.001 |
| **PFS** | 구성요소 8개 | 0.725 (0.707) [0.649–0.791] | 0.98 / 0.86 | 0.736 / 0.726 | p<0.001 |
| **LRRFS** | 구성요소 8개 | 0.714 (0.682) [0.630–0.794] | 1.43 / 1.15 | 0.759 / 0.727 | p<0.001 |

> Supplement(복합 병기 mTstage·mNstage): DSS 0.859(0.854) / OS 0.803(0.799) / PFS 0.718(0.712) / LRRFS 0.697(0.689).
