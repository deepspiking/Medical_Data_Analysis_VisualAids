...
> 🔄 2026-09-18 갱신: **Nomogram = 주요변수 5개(PD·deposit·DOI·size·bilateral)** primary + **복합 병기 companion**(§임상 도구).
...
> **정직성 각주 — mNstage**: 수정병기의 N축 개정(mNstage vs N stage)은 **ΔC≈0으로 구분력 개선이 없어** 위 표에서 제외했습니다. 수정병기 우월성은 **T축 개정(mTstage, PD_01vs2 반영)이 주도**하며 mStage의 개선도 사실상 mT 개정에서 비롯됩니다 (상세: `정세운선생님_mNstage_답변_20260903.md`). (2026-09-17 #52 mNstage 정정 반영 후에도 ΔC: PFS +0.003 / DSS −0.002 / OS −0.006 / LRRFS +0.010 — 전부 CI가 0 포함, 결론 동일.) mNstage NRI(LRRFS) 민감도·다중비교 분석은 `정세운선생님_mNstage_NRI_민감도_20260918.md` — **Holm p=0.047·permutation p=0.054로 취약(exploratory)**.
...
- 원격전이(DM) 9건 중 7건이 PD-HC에서 발생 (**25.9% vs 1.9%**) → 수정병기의 핵심 지표인 PD_01vs2가 전이 위험을 실질적으로 반영
...
- 4개 endpoint 모두 **주요변수 5개(PD_01vs2·LN deposit·DOI·size·bilateral)** 로 구성(stage 변수 제외)
- **복합 병기(mTstage·mNstage) 버전도 본문에 동반**(companion)
...
> 주요변수 = **PD_01vs2, LN deposit, DOI, tumor size, bilateral**(stage 제외). 선생님 결정: 개별 변수
...
- Calibration slope 0.92~1.61(대체로 1 근처), intercept ≈0 → **보정 양호**; DCA에서 대부분 구간 nomogram > treat-all/none.
- 한계: 단일기관 n=133 → **외부검증 필요**, 변수선택 낙관성은 optimism-corrected C로 일부 보정.

### OS

![그림 4. OS Nomogram (주요변수) — 개별 환자 3·5년 생존확률](results/exp1/nomogram/nomogram_OS.png)
...
![그림 7. PFS Nomogram (주요변수) — 개별 환자 3·5년 생존확률](results/exp1/nomogram/nomogram_PFS.png)
...
![그림 10. DSS Nomogram (주요변수)](results/exp1/nomogram/nomogram_DSS.png)
...
![그림 13. LRRFS Nomogram (주요변수)](results/exp1/nomogram/nomogram_LRRFS.png)
...
> *"병기에 PD_01vs2 한 가지만 더하면 — 교란 보정 후에도 — 4개 생존지표 전부에서 기존 병기보다 재분류가 개선되고, 고위험군은 평균 10~21개월 일찍 사건이 발생한다(모두 p<0.001). 특히 DSS 구분력 +0.105는 이 데이터에서 가장 큰 예후 개선폭이다."*
