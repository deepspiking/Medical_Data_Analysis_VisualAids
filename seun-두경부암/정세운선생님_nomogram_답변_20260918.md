...
## 6. 참고 — "어떤 구성요소가 가장 중요한가?" (stage 제외)

병기 nomogram에는 중복 방지를 위해 composite(mT·mN)만 넣었지만, 선생님 제안대로
**stage를 전부 빼고 구성요소만 넣은 모델**로 개별 인자 중요도를 확인했습니다
(상세: `정세운선생님_구성요소_중요도_20260918.md`).

- **PD cellularity가 4개 endpoint 중 3개에서 1위**(LRRFS 2위), 임상인자 포함해도 **4/4 1위**.
- 그 뒤로 **LN deposit · DOI · tumor size**. PD의 p: DSS/OS/PFS <0.001, LRRFS 0.007.
- → **수정 T staging(PD-high를 mT4로 상향)의 근거를 정면으로 지지**.

![그림 13. 구성요소 중요도 (stage 제외, |β|×SD)](results/exp1/component_importance/component_importance.png)

> 주의: 구성요소 간 상관(size↔DOI, count↔deposit)으로 개별 p값은 불안정 →
> 순위는 **모델 내 상대 기여도**로 해석.
