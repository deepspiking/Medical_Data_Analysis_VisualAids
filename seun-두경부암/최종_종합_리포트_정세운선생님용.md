...
> 🔄 2026-09-18 추가: **Nomogram + Calibration + DCA** (§7) — 그림 포함
...
| **개별 환자 예후 예측도구로 쓸 수 있나?** | ✅ Nomogram 4종 제공 | DSS **0.878** / OS **0.809** / PFS **0.725** / LRRFS **0.714** — §7 |
...
## 7. 임상 예측도구 — Nomogram + Calibration + DCA (신규, 2026-09-18)

> 선생님 요청으로 **개별 환자 3·5년 생존확률을 계산하는 nomogram**과 보조 figure
> (Calibration·DCA·C-index·time-AUC·risk-group KM)를 구현. 상세: `정세운선생님_nomogram_답변_20260918.md`
> · 코드: `nomogram_analysis.py` · 그림: `results/exp1/nomogram/`

**표 7. Nomogram 성능 (n=133)**
...
- 선생님 결정: T stage와 구성요소가 겹치므로 **개별 변수 중요도를 강조하는 구성요소(stage 제외) nomogram을 primary**로;
  복합 병기(mTstage·mNstage) 버전은 supplement(`nomogram_*_stage.png`). 해석(축 길이)=`nomogram_해석_가이드.md`.
- **DSS가 가장 강하고(C 0.88), LRRFS가 가장 약함(C 0.71)** — endpoint 난이도 차이.
...
![그림 N1. OS Nomogram — 개별 환자 3·5년 생존확률](results/exp1/nomogram/nomogram_OS.png)

![그림 N2. OS Calibration (A: 3년, B: 5년)](results/exp1/nomogram/panels/panel_OS_calibration.png)

![그림 N3. OS 평가 패널 (A: DCA, B: time-dependent ROC, C: risk-group KM)](results/exp1/nomogram/panels/panel_OS_decision.png)

![그림 N4. PFS Nomogram — 개별 환자 3·5년 생존확률](results/exp1/nomogram/nomogram_PFS.png)

![그림 N5. PFS Calibration (A: 3년, B: 5년)](results/exp1/nomogram/panels/panel_PFS_calibration.png)

![그림 N6. PFS 평가 패널 (A: DCA, B: time-dependent ROC, C: risk-group KM)](results/exp1/nomogram/panels/panel_PFS_decision.png)

![그림 N7. DSS Nomogram — 개별 환자 3·5년 생존확률](results/exp1/nomogram/nomogram_DSS.png)
...
![그림 N10. LRRFS Nomogram — 개별 환자 3·5년 생존확률](results/exp1/nomogram/nomogram_LRRFS.png)
